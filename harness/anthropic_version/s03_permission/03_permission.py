...
def run_bash(command: str) -> str:
    try:
        r = subprocess.run(command,shell=True,cwd=SCRIPT_DIR,capture_output=True,timeout=120)
...
def check_deny_list(command:str) ->str | None:
    for pattern in DENY_LIST:
        if pattern in command:
            return f"Error: Denied ({pattern})"
    return None

# Gate 2: Rule matching — context-dependent checks
PERMISSION_RULES = [
    {"tools": ["write_file", "edit_file"],
     "check": lambda args: not (SCRIPT_DIR / args.get("path", "")).resolve().is_relative_to(SCRIPT_DIR),
